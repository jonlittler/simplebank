...
func TestInvalidJWTTokenAlgNone(t *testing.T) {
	...
	/* create new payload - directly */
	payload, err := NewPayload(TokenTypeAccessToken, util.RandomOwner(), time.Minute)
	require.NoError(t, err)

	/* build token with dummy payload - directly */
	...
	/* verify token - using token with dummy payload */
	payload, err = maker.VerifyToken(TokenTypeAccessToken, token)
	require.Error(t, err)
